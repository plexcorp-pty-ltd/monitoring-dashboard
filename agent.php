...
if ($_ENV['MASTER_NODE'] == 'yes') {
    try {
        ...
        $model->queryNoResult("DELETE FROM server_stats WHERE dt_datetime < ?", [$clearWindow]);
        $model->queryNoResult("DELETE FROM server_spikes WHERE dt_datetime < ?", [$clearWindow]);
        $model->queryNoResult("DELETE FROM emails_sent_today WHERE date_no_time != ?", [date("Y-m-d")]);
    } catch (\Exception $e) {
        print $e->getMessage();
    }
    ...
    if ($_ENV['SMTP_HOST'] != null && $_ENV['SMTP_HOST'] != "null" && !empty($_ENV['SMTP_HOST'])) {
        $emailsSentToday = $model->emailsSentToday();
        if ($emailsSentToday < (int) $_ENV['MAX_THRESHOLD_EMAILS_PER_DAY']) {
            $hosts = $model->getHostnames();
            ...
            foreach($hosts as $host) {
                try {
                    $spikes = $model->getSpikes($since, $host['hostname']);
            ...
            if (count($alarms) > 0) {
                ...
                $model->saveData("emails_sent_today", ["date_no_time" => date("Y-m-d")]);
